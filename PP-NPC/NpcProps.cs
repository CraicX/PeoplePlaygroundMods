	...
	public class Props : MonoBehaviour
	{
		...
		public void Init(PhysicalBehaviour pb)
		{
			...
			if (P.TryGetComponent(out FirearmBehaviour FBH))
			{
				...
				Traits["gun"]           = true; 
				Traits["handgun"]       = P.HoldingPositions.Length == 1;
			...
			else if (P.TryGetComponent<NpcChip>(out NpcChip chip))
			{
				angleHold    = -95f;
				angleAim     = 180f;
				canRecruit   = chip.ChipType == Chips.AI;
				canUpgrade   = chip.ChipType != Chips.AI;
				canStrike    = false;
			} 

			if ( P.name.ToLower().Contains( "submachine gun" ) )
			{
				needsTwoHands = false;
			}
			...
			isWeapon = (canStab || canSlice || canStrike || canShoot || canExplode);
		}
	}
}
